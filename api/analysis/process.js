 ...
 */
// 히스토리검색
router.get('/analysis/process', async (req, res) => {
    let conn;
    try {
        ...
        if (processId !== undefined && processId !== 'AU') {
            processIfWhere = `AND v.manufacturing_process_id = ${processId}`
            departmentGroupBy = `group by v.id \n order by v.id asc`;
            selectList = 'manufacturing_process_name processName, v.description locationName, ';

        } else {
            departmentGroupBy = `group by v.manufacturing_process_id \n order by v.manufacturing_process_id asc`;
            selectList = 'v.manufacturing_process_name processName, ';
        }
